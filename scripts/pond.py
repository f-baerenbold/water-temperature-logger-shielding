...
if period == 'all':
    result_folder = "./figures/pond_all/"
else:
    result_folder = "./figures/pond_june/"
...
hobo_shield['Ch: 1 - Temperature Avg : Avg (°C )'] = hobo_shield['Ch: 1 - Temperature Avg : Avg (°C )'] + 0.005 # Calibration of Fall 2022 (sensor 480)
...
hobo_noshield['Ch: 1 - Temperature Avg : Avg (°C )'] = hobo_noshield['Ch: 1 - Temperature Avg : Avg (°C )'] + 0.035 # Calibration of Fall 2022 (sensor 482)

# Vemco sensor with shield
vemco_shield_file = [x for x in files if "vemco_pond_shield" in x]
vemco_shield = pd.read_csv(data_folder + vemco_shield_file[0], header=0, skiprows=7, delimiter=',',usecols = [0,1,2],encoding = "ISO-8859-1")
vemco_shield['time'] = pd.to_datetime(vemco_shield['Date(yyyy-mm-dd)'] + vemco_shield['Time(hh:mm:ss)'], format='%Y-%m-%d%H:%M:%S') - timedelta(hours=2)
vemco_shield['timestamp'] = vemco_shield['time'].apply(lambda x: datetime.timestamp(x))
vemco_shield['Temperature (°C)'] = vemco_shield['Temperature (°C)'] + 0.057 # Calibration of Fall 2022

# Vemco sensor without shield
vemco_noshield_file = [x for x in files if "vemco_pond_noshield" in x]
...
vemco_noshield['time'] = pd.to_datetime(vemco_noshield['Date(yyyy-mm-dd)'] + vemco_noshield['Time(hh:mm:ss)'], format='%Y-%m-%d%H:%M:%S')
vemco_noshield['timestamp'] = vemco_noshield['time'].apply(lambda x: datetime.timestamp(x))
vemco_noshield['Temperature (°C)'] = vemco_noshield['Temperature (°C)'] + 0.021 # Calibration of Fall 2022

# RBR sensor with shield
rbr_shield_folder = [x for x in files if "rbr_pond_shield" in x][0]
rbr_shield_file = [x for x in os.listdir(data_folder + rbr_shield_folder) if "data" in x][0]
rbr_shield = pd.read_csv(data_folder + rbr_shield_folder + "/" + rbr_shield_file)
rbr_shield['time'] = pd.to_datetime(rbr_shield['Time'], format='%Y-%m-%d %H:%M:%S.%f')
rbr_shield['timestamp'] = rbr_shield['time'].apply(lambda x: datetime.timestamp(x))
rbr_shield['Temperature'] = rbr_shield['Temperature'] #+ 0.004 # Calibration of Fall 2022

# RBR sensor without shield
rbr_noshield_folder = [x for x in files if "rbr_pond_noshield" in x][0]
rbr_noshield_file = [x for x in os.listdir(data_folder + rbr_noshield_folder) if "data" in x][0]
rbr_noshield = pd.read_csv(data_folder + rbr_noshield_folder + "/" + rbr_noshield_file)
rbr_noshield['time'] = pd.to_datetime(rbr_noshield['Time'], format='%Y-%m-%d %H:%M:%S.%f')
rbr_noshield['timestamp'] = rbr_noshield['time'].apply(lambda x: datetime.timestamp(x))
rbr_noshield['Temperature'] = rbr_noshield['Temperature'] #+ 0.003 # Calibration of Fall 2022
...
temp_rbr_shield = np.interp(timestamp,rbr_shield['timestamp'],rbr_shield['Temperature'])
temp_rbr_noshield = np.interp(timestamp,rbr_noshield['timestamp'],rbr_noshield['Temperature'])
...
fig1.savefig(result_folder + 'Hobo pond.png',format='png',dpi=300)
...
ax2.set_ylabel('Temperature difference [°C]', color = 'blue',size= label_font)
ax2b.set_ylabel('Solar radiation [W m$^{-2}$]',color='darkorange', size= label_font)
fig2.savefig(result_folder + 'Vemco pond.png',format='png',dpi=300)
...
ax1.plot(time,temp_rbr_noshield,linestyle='dashed')
ax1.plot(time,temp_rbr_shield)
...
ax2.plot(time,temp_rbr_noshield - temp_rbr_shield, label='Surface temperature difference')
...
ax2.set_ylabel('Temperature difference [°C]', color = 'blue',size= label_font)
ax2b.set_ylabel('Solar radiation [W m$^{-2}$]',color='darkorange', size= label_font)
fig3.savefig(result_folder + 'RBR pond.png',format='png',dpi=300)
...
ax1.tick_params(axis='both', which='major', labelsize=label_font)
ax1.set_title('Hobo (pond)',fontsize=label_font)
...
print(f'\nHobo intercept {model_hobo.intercept_}\n')
...
ax2.tick_params(axis='both', which='major', labelsize=label_font)
ax2.set_title('Vemco (pond)',fontsize=label_font)
...
sct3 = ax3.scatter(swr_mov,temp_rbr_noshield - temp_rbr_shield,c=time_hours,s=5,cmap='Greens')
ax3.set_xlabel('Incoming solar radiation [W m$^{-2}$]', size= label_font)
#ax3.set_ylabel('Temperature excess [°C]', size= label_font)
ax3.set_ylim(-0.2,1.0)
ax3.tick_params(axis='both', which='major', labelsize=label_font)
ax3.set_title('RBR (pond)',fontsize=label_font)

corr_rbr = round(ma.corrcoef(ma.masked_invalid(swr_mov), ma.masked_invalid(temp_rbr_noshield - temp_rbr_shield))[1,0]*1000)/1000
model_rbr = LinearRegression().fit(np.array(swr_mov).reshape((-1, 1)), np.array(temp_rbr_noshield - temp_rbr_shield))

textstr = 'R$^2$=$%.2f$\nSlope=$%.1e$' % (corr_rbr,model_rbr.coef_[0])
ax3.text(0.05, 0.95, textstr, transform=ax3.transAxes, fontsize=label_font,
            verticalalignment='top', bbox=props)

cb = plt.colorbar(sct3,ax=ax3)
cb.set_label(label='Hour of Day',size=label_font-1)

plt.tight_layout()
fig4.savefig(result_folder + 'solar correlation pond.png',format='png',dpi=300)
...
print('Mean overestimation of RBR: ', np.round(np.mean(temp_rbr_noshield - temp_rbr_shield),3))
print('\n')
...
print('Mean overestimation of RBR (only daytime): ', np.round(np.mean(temp_rbr_noshield[swr>10] - temp_rbr_shield[swr>10]),3))
print('\n')
...
print('Max overestimation of RBR: ', np.round(np.max(temp_rbr_noshield - temp_rbr_shield),3))
print('\n')
print('Min/mean/max water temperature: ', np.round(np.min(temp_rbr_noshield),3), np.round(np.mean(temp_rbr_noshield),3), np.round(np.max(temp_rbr_noshield),3))
print('\n')